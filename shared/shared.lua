...
function LGF:GetDependency(resource_name, required_version)
    ...
    if current_version ~= required_version then
        return false, LGF:DebugValue(("^1[ERROR]^7 The version of resource ^1[%s]^7 does not match the required version. Required: ^3%s^7"):format(resource_name, required_version))
    end
    return true
end
